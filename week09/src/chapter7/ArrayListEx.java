...
public class ArrayListEx {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<4; i++) {
			...
			String s = scan.next(); //키보드로부터 이름을 입력
		...
		//가장 긴 이름 출력
		int logestIndex = 0; //현재 가장 긴 이름이 있는 ArrayList 내의 인덱스
		for(int i=0; i<a.size(); i++) {
			if(a.get(logestIndex).length() < a.get(i).length()) { //이름 길이 비교
				logestIndex = i; //i번째 이름이 더 긴 이름임
			}
		}
